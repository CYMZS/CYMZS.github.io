...
**最右推导**：每次总是选择最右侧的符号进行替换，又称为**规范推导**
...
状态3是该移进还是规约，可以使用Follow集合来判断，发现$Follow(E)=\{\varepsilon\}$，因此$E$后面可能跟$+$号,所以不能能规约
...
* 其他和LR(0)相同，仅闭包的计算不同
  * 对于项目$X\to\alpha.Y\beta,a$
    * 添加$Y\to.\gamma,b$ 到项目集，$b\in FIRST\_S(\beta a)$
...
* YACC is Yet Another Compiler-Complier
* 1975年由Steve Johnson在UNIX上实现
* 后来很多工具再次基础上做了改进
  * GNU Bison
* IEEE Posix P1003.2
...
语法规则：上下文无关文法的规则及相应的语义动作

%%
用户代码：用户提供的代码
```

`src.y`->`Yacc`->`.c`->`gcc`->`.out`，这个`.out`就是分析器
